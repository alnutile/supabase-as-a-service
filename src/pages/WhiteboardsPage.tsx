...
import { CollectionIcon, LockIcon, PlusIcon, UsersIcon, WhiteboardIcon } from '../components/icons'
import { formatDate } from '../lib/util'

type Whiteboard = Database['public']['Tables']['whiteboards']['Row']
type Collection = Database['public']['Tables']['collections']['Row']

// Planner list page: the workspace's Excalidraw whiteboards. Mirrors
// ArtifactsPage (list + create → editor route + collection filter + the shared
// AddToCollectionBar), leaner because a board's body lives in the editor.
export default function WhiteboardsPage() {
  ...
  // Live list: reflect a teammate's new/renamed board (and the AI's) without a
  // refresh, same postgres_changes convention as the rest of the app.
  useEffect(() => {
    const channel = supabase
      .channel('whiteboards-list')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'whiteboards' }, () => load())
      .subscribe()
    return () => {
  ...
  async function create() {
    ...
    const { data } = await supabase
      .from('whiteboards')
      .insert({ owner_id: user.id, title: 'Untitled whiteboard', scene: {}, visibility: 'private' })
      .select()
      .single()
    setCreating(false)
    if (data) navigate(`/whiteboards/${data.id}`)
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-4xl px-6 py-8">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-text">Whiteboards</h1>
              <p className="mt-1 text-sm text-muted">
                Plan on an Excalidraw canvas — draw together in real time, and the assistant can read
                and draw on it too. Group boards into collections to chat with them.
              </p>
            </div>
            <button
              onClick={create}
            ...
            >
              <PlusIcon className="h-4 w-4" /> New whiteboard
            </button>
          </div>
          ...
          {visible.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-border py-16 text-center">
              <WhiteboardIcon className="mx-auto mb-3 h-8 w-8 text-faint" />
              <p className="text-sm text-muted">
                {activeId ? 'No whiteboards in this collection yet.' : 'No whiteboards yet.'}
              </p>
              <button
                onClick={create}
                disabled={creating}
              ...
              {visible.map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button onClick={() => navigate(`/whiteboards/${b.id}`)} className="flex flex-1 flex-col text-left">
                      <div className="mb-3 flex h-20 items-center justify-center rounded-lg border border-border bg-surface-2 text-faint">
                        <WhiteboardIcon className="h-7 w-7" />
                      </div>
                      ...
                      <p className="mt-1 text-xs text-faint">Updated {formatDate(b.updated_at)}</p>
                    </button>
                  </div>
                )
              })}
      ...
      <AddToCollectionBar kind="whiteboard" selectedIds={[...selected]} onClear={() => setSelected(new Set())} />
    </div>
  )
}
