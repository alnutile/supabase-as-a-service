...
- **External MCP client (outbound):** the inverse of the MCP *server* above — the
  ...
  **Outbound OAuth (connect to sign-in servers, migration 0111):** besides a static bearer token, a
  server can authenticate via the **OAuth 2.1 + PKCE authorization-code flow with Dynamic Client
  Registration** — the client mirror of the inbound `mcp-oauth` server (0077), for endpoints like
  Notion / Linear / GitHub / Atlassian that require a login instead of a pasted token. An admin picks
  "OAuth (sign in)" when adding a server; the admin-only **`mcp-connect`** function (`verify_jwt: true`)
  discovers the server's endpoints (RFC 9728 `WWW-Authenticate`/protected-resource metadata → RFC 8414
  auth-server metadata), dynamically registers a public PKCE client (RFC 7591, or takes manual
  endpoints/client-id for servers without DCR), stores the pending state + PKCE verifier
  (`mcp_oauth_states`), and returns the `/authorize` URL the browser redirects to. The public
  **`mcp-oauth-callback`** function (`verify_jwt: false` — the provider's redirect carries no session)
  exchanges the code for tokens and marks the server connected. Same Vault trust boundary as bearer:
  `mcp_servers.secret_id` now holds the **access** token (nullable until connected), `refresh_secret_id`
  the refresh token, `client_secret_id` an optional confidential-client secret; non-secret metadata
  (endpoints, client_id, `expires_at`, `status`) lives in the `mcp_servers.oauth` jsonb, `auth_type` is
  `'bearer'|'oauth'`. `_shared/mcp.ts` reads a live bearer through `ensureAccessToken`
  (`_shared/mcp_oauth.ts`), which **refreshes on expiry** transparently, so every agent loop that calls
  a remote MCP tool works unchanged. Secrets flow only through service-role RPCs (`read_mcp_oauth` /
  `save_mcp_oauth_config` / `save_mcp_oauth_tokens`). Pure logic (WWW-Authenticate parsing, metadata-URL
  derivation, expiry math, authorize-URL building, PKCE challenge) is unit-tested in
  `tests/mcp_oauth_test.ts`. *(Planned: a "Connections" view to revoke; per-user OAuth servers via the
  0036 owner_id/scope columns; auto-refresh of the tool cache.)*
