...
- **Card boards (Planner — free-form card walls, migration 0075):** the second Planner
  ...
  pastel colors (solid, dark text) read the same in light/dark. **Card size is the second
  visual axis (migration 0121):** if position is the ranking, SIZE is the weight — drag the
  bottom-right grip to resize freely, or pick a named preset (small/medium/large/huge) from
  the card's size button, and the body text scales with the width so a big card reads as a
  big idea rather than a big empty rectangle. `w`/`h` ride the SAME `cards` jsonb (no schema
  change; a pre-0121 card has none and renders at the default), they're part of `cardsSig` so
  a peer's resize syncs like a move, and both the position and the size are clamped to the
  canvas so a grown card can't escape it. The maths is the pure, unit-tested
  `src/lib/cardBoard.ts` (`clampSize`/`clampPosition`/`resizeTo`/`cardFontSize`/`sizeLabel` +
  the card normalization and signature) — mirrored server-side in `_shared/card_board.ts`
  (`normSize` accepts a named `size` or explicit `w`/`h` and clamps it, `sizeLabel` names it
  back), so `add_cards`/`create_card_board` take a `size` and `cardsToText` tags a resized
  card (`- [blue, large] …`) and the emphasis survives into the prompt. 0121 only re-seeds
  those two tool schemas. `CardBoardsPage` (`/cards`)
...
- **Repositories (GitHub — the company's code as memory, migration 0124):** `RepositoriesPage`
  (route `/repositories`, sidebar under **Assets**) connects the GitHub repositories behind the
  company's products so the workspace can READ them and keep ONE maintained **summary artifact per
  repo** — what the product is, who it is for, how it is built, the key areas of the code, what the
  team is working on now, open work, glossary, unknowns. That artifact is the answer; the repo is the
  evidence (the compiled-first discipline of the knowledge compiler applied to code). A
  `repositories` row is `provider` + `full_name` (owner/name) + cached metadata + `artifact_id` +
  the sync bookkeeping (`last_synced_at`/`last_sync_sha`/`last_sync_status`/`sync_summary`, the
  last pass's change brief); visibility mirrors links (`private`/`workspace`, default workspace
  because the point is a shared picture), realtime-published so a sync flips live for everyone.
  **Token:** a single workspace GitHub token, admin-set in **Settings → GitHub**, Vault-backed
  exactly like Dropbox (`set_github_integration` / service-role-only `read_github_secret` /
  `github_is_configured`); public repos work without one (anonymous rate limit), private ones need
  it; the token never reaches a browser or a model. **Six seeded `is_builtin` tools** (handlers in
  `_shared/repositories.ts`; the MCP server delegates the same names via `runBuiltin`):
  `add_repository` (URL or owner/name → metadata → row; optional collections; does NOT sync),
  `list_repositories`, `get_repository` (metadata + the summary artifact + GitHub activity since
  the last sync or `since` — commits/open PRs/open issues; read this FIRST when asked about a repo),
  `browse_repository` (list a directory / read one text file, capped — the fallback for specifics),
  `sync_repository` (the point: reads README + up to 10 key files — `CLAUDE.md`/`AGENTS.md`/
  manifests/Dockerfile/docs — + the tree layout + languages + commits since last sync + open
  PRs/issues into a budgeted digest, asks the orchestrator model to write or **REVISE IN PLACE**
  the summary against a fixed outline, files the artifact into the repo's collections, records the
  pass, returns a change brief; optional `focus` for a pass that digs into one area), and
  `add_repository_to_collection`. The page's Connect box calls `add_repository` then
  `sync_repository` through `run-tool`; a "Chat" button deep-links `/chat?artifact=:id` so the
  summary is open beside the thread. Pure logic (ref parsing, key-file selection, tree/commit
  rendering, the digest budget, the sync prompt, splitting the reply into document + brief) is
  `_shared/github.ts` (unit-tested `tests/github_test.ts`); the browser mirror is
  `src/lib/repositoryRef.ts` (unit-tested). Repository content is passed to the model as
  **untrusted material** (README/issue/commit text is data, never instructions). The summary
  artifact is owned by the repo's owner and its visibility follows the repo (workspace → `unlisted`,
  private → `private`); `collection_repositories` (mirror of `collection_links`, learned by the
  generic `collection.item_added` trigger and the 0122 visibility propagation) files a repo into a
  collection, and `loadCollectionsContext` injects a compact repositories block AND folds each
  repo's summary artifact into the budgeted artifacts even when only the repo row was filed. Emits
  `repository.created` / `repository.synced` (change brief in `data.summary`), so a listener can
  route a brief to Slack; a nightly refresh is an agent scoped to `sync_repository` on a `schedules`
  row. Model usage logs with context `repository`. Full reference: `docs/repositories.md`.
  *(Planned: GitLab/Bitbucket/CodeCommit behind the same `provider` column; a GitHub webhook
  target so a push triggers `sync_repository`; per-repo tokens; compiling repo summaries into
  knowledge pages and a workspace "what we build" profile.)*
...
- **Settings & feature flags (migration 0065):** Settings is a sidebar SECTION
  ...
  Slack, Dropbox, GitHub (the Repositories token), External MCP, Invite people, and **Feature flags** (admin). The sidebar is now driven
...
src/
  ...
  pages/                       LoginPage, ChatPage, ArtifactsPage,
                               ...
                               RepositoriesPage (GitHub repos → maintained summary artifacts),
  ...
  lib/
    ...
    cardBoard.ts               Pure Cards-board geometry: card size clamping,
                               the corner-grip resize maths, canvas-bounded
                               positions, text scaling, presets (unit-tested)
    ...
    repositoryRef.ts           Pure Repositories helpers: GitHub ref parsing (browser
                               mirror of _shared/github.ts), search, sync labels
    repositories.ts            run-tool client for add_repository / sync_repository
...
supabase/
  ...
  functions/_shared/github.ts  Pure Repositories core: repo ref parsing, key-file selection, tree/commit/PR rendering, the budgeted digest, the sync prompt + reply splitting (unit-tested in tests/github_test.ts)
  functions/_shared/repositories.ts  GitHub REST client (Vault token) + the six repository builtins (add/list/get/browse/sync/add_to_collection), shared by every agent loop and the MCP server
...
`usage_events`
(per-call token/cost accounting), `repositories` + `collection_repositories` (connected GitHub
repos and their summary artifacts), `user_tables` (registry for the Tables feature;
