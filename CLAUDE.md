...
- **Unified inbox (messages from any source, migration 0064):** one place to push every
  ...
  **IMAP pull connector (migration 0083):** the first *pull* source (everything above is
  *push*). A member connects a real mailbox (Gmail app-password, Fastmail, a work IMAP
  server) in **Settings → Email accounts** — an `imap_accounts` row (personal, owner-scoped
  RLS, `private`/`workspace` visibility for the mail it pulls) with a Vault-backed password
  (same posture as the 0016 email integration: written via `set_imap_account`, read only by
  the service role via `read_imap_secret`) and a per-account UID cursor + poll interval.
  Edge functions are request-scoped and can't hold a live IMAP socket, so the public
  **`imap-poll`** edge function (`verify_jwt: false`) does **poll-and-disconnect on a cron
  tick** (same `x-cron-secret` convention as the scheduler): connect over TLS → LOGIN →
  SELECT → `UID SEARCH` newer than the cursor → `UID FETCH` a bounded batch → parse with
  `postal-mime` → insert into `inbox_messages` (`source='email'`, dedup on the
  `(source, external_id)` = Message-ID index) → optional `\Seen`. A second entry point lets
  an owner POST `{account_id}` (session JWT / `mcp_tokens`, checked in code) to poll one
  account on demand ("Poll now" / connection test). The RFC 3501 wire-parsing (literal-aware
  line/byte reader, `parseSearchUids`/`parseSelect`/`selectNewUids`) lives in the pure,
  unit-tested `supabase/functions/_shared/imap.ts` (`tests/imap_test.ts`) — no heavy
  Node IMAP dependency in the edge runtime. **Deploy step:** apply the out-of-band
  `cron.schedule(...)` block in the migration once (substituting the project ref, same as the
  scheduler in 0010). *(Planned: Google/Microsoft OAuth account connectors; injecting a
...
- **MCP server:** `supabase/functions/mcp/index.ts` is a JSON-RPC-over-HTTP MCP
  ...
  **Claude Code** connects directly (`claude mcp add --transport http …`); **Claude
  ...
  cases. **Collection as a two-way hub:**
