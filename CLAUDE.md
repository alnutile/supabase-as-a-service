...
- **Memory (per-user facts the AI remembers, migration 0069):** `MemoryPage`
  (route `/memory`, sidebar under **Assets**, any member) is where the assistant
  keeps durable facts about *you* — preferences, role, timezone — so it can serve you
  better across separate conversations. A `user_memories` row is one fact (`content` +
  an optional short `key`). Unlike todos/links/collections (private-OR-workspace),
  memory is deliberately **owner-only** (same trust model as `conversations`/`messages`):
  it's personal context, not shared content, so there's no workspace mode and no
  collections join. An optional `key` (slugified, e.g. `timezone`) plus a `unique (owner, key)`
  constraint lets the assistant **overwrite** a named fact instead of piling up duplicates
  (NULL keys are distinct, so unkeyed memories accumulate freely). The **chat function injects the caller's memories into the system prompt**
  (`loadUserMemories` → `formatMemoriesForPrompt` in `_shared/memory.ts`) so they actually
  shape replies. Seeded `is_builtin` tools `remember` / `list_memories` / `update_memory` /
  `forget` (in `_shared/builtins.ts`, so chat/scheduler/webhook agents + the universal
  `run-tool` runner all manage memory) log `memory.saved`/`.updated`/`.forgotten`. Pure
  validation/normalization/formatting lives in `_shared/memory.ts` and is unit-tested
  (`tests/memory_test.ts`); `user_memories` is realtime so tool-written memories appear
  live on the page. *(Planned: MCP-server parity for an external Claude; scoping injection
  to scheduler/webhook agent runs.)*
