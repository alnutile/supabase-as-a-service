...
- **To-dos:** `TodosPage` (route `/todos`, sidebar under **Assets**, any member) is a
  ...
  **Team-visible to-dos over the tool surface (migration 0124):** `todos.visibility`
  (`private`/`workspace`) has existed since 0041 and the REST API always took it, but
  `create_todo` **hardcoded `private`** — so every to-do an agent, the MCP server, the
  `supanet` CLI or a `run-tool` call filed was owner-only, with no argument to share it and
  no way to promote one afterwards. `create_todo` / `update_todo` now take `visibility`
  (0124 only re-seeds those two tool schemas; no DB change). The vocabulary is the pure,
  unit-tested `_shared/todos.ts` (`normalizeTodoVisibility`), shared by the builtins AND the
  REST function so the two can't drift: it accepts `team`/`shared` as aliases for
  `workspace` (the words a person says, so the words a model repeats), and an unrecognised
  value is an **error, never a silent fall back to `private`** — the old REST code ignored a
  bad value, which let a script report success while the to-do stayed invisible to everyone
  but its owner. Two deliberate asymmetries: the default stays `private` (an agent filing
  work AT one person shouldn't quietly publish it to the team — sharing is a choice someone
  makes), and `update_todo`'s visibility change is **owner-only** even though any member can
  re-lane or complete a workspace to-do, so one member can't pull a shared to-do back to
  private and hide it from everyone else. `list_todos` tags the shared ones `team` so "share
  these with the team" needs no second lookup. Migration 0122 still covers the collection
  case (filing an item into a workspace collection promotes it), so this argument is really
  for the ad-hoc team task that lives outside a collection.
